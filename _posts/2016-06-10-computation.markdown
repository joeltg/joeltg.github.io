...
![]({{ site.url }}/assets/images/lisp.jpg)
...
What's weird is that even though we know that almost all boolean functions require exponentially large circuits, *we don't know what any of them are*. Everything that we have a name for - every point in concept-space that humanity has ever touched - either fits into a polynomial circuit or has yet to be proven. We're not even sure if NP-complete problems like SAT or EXPTIME-complete problems like Go need exponentially large circuits!
...
Rather tautologically, we don't have any examples of things we don't know to describe. But the fact that they have to exist - and that there are so many of them - should bother us.
